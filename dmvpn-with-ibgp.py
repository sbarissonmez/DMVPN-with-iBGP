...
def config_vrf(task):
    vrf_template = task.run(
        task=template_file, name="Building VRF Configuration", template="vrf.j2", path="./templates")
    task.host["vrf"] = vrf_template.result
    vrf_output = task.host["vrf"]
    vrf_send = vrf_output.splitlines()
    task.run(task=netmiko_send_config,
             name="Pushing VRF Commands", config_commands=vrf_send)
    config_dmvpn(task)


def config_dmvpn(task):
    dmvpn_template = task.run(
        task=template_file, name="Building DMVPN Configuration", template="dmvpn.j2", path="./templates")
    task.host["dmvpn"] = dmvpn_template.result
    dmvpn_output = task.host["dmvpn"]
    dmvpn_send = dmvpn_output.splitlines()
    task.run(task=netmiko_send_config, name="Pushing DMVPN Commands",
             config_commands=dmvpn_send)
    config_bgp(task)


def config_bgp(task):
    bgp_template = task.run(
        task=template_file, name="Building BGP Configuration", template="bgp.j2", path="./templates")
    task.host["bgp"] = bgp_template.result
    bgp_output = task.host["bgp"]
    bgp_send = bgp_output.splitlines()
    task.run(task=netmiko_send_config,
             name="Pushing BGP Commands", config_commands=bgp_send)
